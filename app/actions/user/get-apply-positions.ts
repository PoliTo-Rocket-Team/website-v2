...
export type ApplyPosition = Database["public"]["Tables"]["apply_positions"]["Row"] & {
...
export async function getApplyPositionsByUserRole() {
  ...
  // Create a mapping of division_id to subteam_id
  const divisionToSubteamMap = new Map();
  divisions.forEach(division => {
    if (division.subteam_id) {
      divisionToSubteamMap.set(division.id, division.subteam_id);
    }
  });
    ...
    if (userRole.type === "president") {
      allPositions.forEach(position => {
        filteredPositions.push({
          ...position,
          divisions: position.divisions as Partial<Database["public"]["Tables"]["divisions"]["Row"]> | null
        });
      });
    } 
    // Chiefs and coordinators can see positions in all divisions that belong to their subteam
    else if ((userRole.type === "chief" || userRole.type === "coordinator") && userRole.subteam_id) {
      // Get all division IDs that belong to this subteam
      const divisionsInSubteam = subteamToDivisionsMap.get(userRole.subteam_id) || [];
      
      allPositions.forEach(position => {
        if (divisionsInSubteam.includes(position.division_id)) {
          filteredPositions.push({
            ...position,
            divisions: position.divisions as Partial<Database["public"]["Tables"]["divisions"]["Row"]> | null
          });
